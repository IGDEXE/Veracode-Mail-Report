...
function Send-VeracodeReport {
    ...
    try {
        ...
        $Password = ConvertTo-SecureString "senhadeApp" -AsPlainText -Force
...
function Get-VeracodeReport {
    ...
    try {
        ...
        # Gera o relatorio
        $out = VeracodeAPI.exe -action DetailedReport -buildid "$buildID" -format pdf -outputfilepath "$caminhoRelatorio"
...
Send-VeracodeReport $caminhoRelatorio